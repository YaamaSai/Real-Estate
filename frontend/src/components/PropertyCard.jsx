...
const PropertyCard = ({ property }) => {
  const propertyLink = `/property/${property._id || property.id}`;
  ...
  return (
    ...
    >
      <div className="relative h-72 overflow-hidden">
        <Link to={propertyLink} className="block w-full h-full">
          <img
      ...
      <div className="p-8 flex-1 flex flex-col">
        ...
        <div className="mt-6 md:hidden">
          <Link
            to={propertyLink}
            className="w-full bg-slate-950 text-white py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:bg-gold-500 hover:text-slate-950 transition-all shadow-lg shadow-slate-950/10 active:scale-[0.98]"
          >
            View Details <ArrowUpRight size={14} className="text-gold-500" />
          </Link>
        </div>
      </div>
    </motion.div>
